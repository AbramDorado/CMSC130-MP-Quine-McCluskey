...
def getVariables(iteration, variable):
    '''
...
def multiplyMinterms(num1,num2): 
    '''
...
def multiplyExpressions(exp1,exp2):
    '''
...
def getMergeMinterms(iteration):
    '''
...
# Auxiliary Function that convertsa tuple into a string 
...
def prime_charts(enter_minterms, all_prime, enter_dontCC, variable):
    ...
    for i in all_prime:
        merged_minterms,y = getMergeMinterms(i), 0
        end =''    
    ...
    if(len(chart) == 0): # If no minterms remain after removing EPI related columns
        final_result = [getVariables(i, variable) for i in EPI] # Final result with only EPIs
    else: # go for further simplification
        P = [[getVariables(j, variable) for j in chart[i]] for i in chart]
        J = [[getVariables(j, variable) for j in chart[i]] for i in chart]
        ...
        try:
            ...
            final_result.extend(getVariables(i, variable) for i in EPI) # Adding the EPIs to final solution
        except Exception as e:
            final_result=(getVariables(i, variable) for i in EPI) # Adding the EPIs to final solution
...
def driver(variable, minterms, dontCare, sizing):
    ...
    try:
        ...
        while True:
            ...
            if (num ==1):
                globals.var4 += convertTuple(tuple)+"\n"
            else:
                globals.vari4 += convertTuple(tuple)+"\n"
            ...
            if should_stop: # If the minterms cannot be combined further
                ...
                globals.txt = convertTuple(tuple02)
                break
